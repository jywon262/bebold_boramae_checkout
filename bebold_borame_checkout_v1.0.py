...
# 2. ★ 중요: 네이버 로그인 쿠키 (개발자도구 F12 -> Network -> Request Headers의 Cookie 값 복사 붙여넣기)
NAVER_COOKIE = "NAC=U4dVBARDeneqB; NNB=IE6SKLJM6PVGS; ASID=3ae9cdcd0000019dfbc856590000001d; ba.uuid=07ff34e8-dd9a-4d5f-aa0d-0db762a9596e; _fbp=fb.1.1779428828717.784145620524393632; nid_inf=-1129236319; NID_AUT=HLM5a4HFETy6ZrvBfQvYV+7z2F5HNixsi4FtzQBE0mz3CnQcmt93WLX3ZBA9SV82; cto_bundle=OD9kJ19uRkpITmlrRVlod3JjNzJTMEh2dWc4VkZ6eENIRWdEZlBGWWhNaWhGbEdUWFJrWkVoWGM2N1hqQSUyRlclMkJIejlSNSUyRjNRdCUyQmgxdEpHT0NhSEdmdXpFZFdIJTJCSHNZVTdzJTJGdHN1T1hMbCUyRlc4TWdSNGJEemh1NDIwQVpXZ1ZsUTZRajlaRjd6aCUyQjF4UGdlNm5TZDhVRVEzOGRBJTNEJTNE; page_uid=jC9SslqosAndl5Bb6pG-477943; ncu=99b95473622449ab8b1976617aace6b4034fa7571f528f; nci4=; ncmc4=8fbf56736c2740f0a94700171af699ea4414e606035ccf1cc62429cf7855907f8047844dafb51477029cbf6ff173c238dbf3fec9130d7b5c48ab3daa675f9d; NACT=1; SRT30=1783651232; NID_SES=; ncvc2=8faf426574325fbd9d631c281bf187ff5707e718501af023da1b27f74eb8; SRT5=1783656421; JSESSIONID=3F06E79698B5BAE43B5DDFC6F931B5C9; BUC=s5SW7joQT0MNOBK1QWn4S3ZJbm8QCSgRcCkR2dHNvrI="

# 3. Supabase API 정보 (설정 페이지에서 확인한 값 넣기)
SUPABASE_URL = "https://gqrcvmlkldvdejplbsdh.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdxcmN2bWxrbGR2ZGVqcGxic2RoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODM2MTkzNTMsImV4cCI6MjA5OTE5NTM1M30.R2ww-seXYkJV7nVGBU_JJqFD9QY8gT2OtURqdXvRnGE"

# 4. ★ 100% 무료 구글 Gemini API 키 입력
GEMINI_API_KEY = "AIzaSyCtVAMq5TwQOyVpotGsicyE7xkF3nspoJU"
# =========================================================================
...
def crawl_once():
    ...
    update_daily_stats_summary(today_str)
    write_sync_log(f"동기화 실행 완료 | 처리 대상: {len(raw_unordered_list)}건 (성공: {success_count}건, 실패: {fail_count}건)")
    return success_count, fail_count, len(raw_unordered_list)

def start_auto_sync_loop():
    print("==================================================")
    print("🚀 Bebold Boramae 수업 예약 동기화 마스터 엔진 가동")
    print("==================================================")
    while True:
        try:
            crawl_once()
        except Exception as e:
            print(f"❌ 루프 예외 감지 백오프 (5초 후 대기 루프 재수행): {e}")
        time.sleep(300)
